...
def train(args):
    ...
    # Init Model
    # Allow disabling plucker embedding via CLI/config: when disabled we pass plucker_emb_dim=0
    model = open_clip_load(args,
    ...
    # Only include plucker_embed parameters when plucker is enabled and the module exists
    if not getattr(args, 'disable_plucker', False) and hasattr(model, 'plucker_embed'):
        parameters.append({"params": model.plucker_embed.parameters(), "lr": float(args.lr)})
    if args.use_lora:
        # Append LoRA-tuned parameters as a parameter group instead of replacing `parameters`
        parameters = [{"params": [p for p in model.parameters() if p.requires_grad], "lr": float(args.lr)}]
