...
def get_sam_model(args, device='cpu', **kwargs):
    ...
    # Freeze all parameters first, then unfreeze the newly added layers
    for p in model.parameters():
        p.requires_grad = False

    # Unfreeze new layers: spatial adapters and plucker patch embed
    if hasattr(model.image_encoder, 'spatial_adapters') and model.image_encoder.spatial_adapters is not None:
        for p in model.image_encoder.spatial_adapters.parameters():
            p.requires_grad = True
    if hasattr(model.image_encoder, 'plucker_embed') and model.image_encoder.plucker_embed is not None:
        for p in model.image_encoder.plucker_embed.parameters():
            p.requires_grad = True
    if hasattr(model.image_encoder, 'neck') and model.image_encoder.neck is not None:
...
def train(args):
    ...
    # Init Model
    model = get_sam_model(args, device=device)
    model.train(True)
    ...
    # SAM feature map dims
    feature_size = args.image_size[0] // model.image_encoder.patch_size
    ...
    parameters = [
        {"params": model.image_encoder.spatial_adapters.parameters(), "lr": float(args.lr)},
    ]
    if not args.disable_plucker:
        parameters.append({"params": model.image_encoder.plucker_embed.parameters(), "lr": float(args.lr)})
    if args.use_lora:
        # Append LoRA-tuned parameters as a parameter group instead of replacing `parameters`
        parameters = [{"params": [p for p in model.image_encoder.parameters() if p.requires_grad], "lr": float(args.lr)}]
            ...
            for eval_idx, eval_data in enumerate(tqdm(eval_dataloader)):
                with torch.no_grad():
                    eval_features, eval_base_features, eval_dense_loss, eval_sparse_loss, eval_regularization_loss, _ = run_iter(
                        model, eval_data, eval_consistency_loss, device=device, base_model=base_model, concat_with_base=args.concat_with_base, separate_forward_regularization=args.separate_forward_regularization, sam=True)
