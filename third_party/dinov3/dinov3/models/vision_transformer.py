...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        **ignored_kwargs,
        ...
        if self.extended:
            ...
            # spatial adapters and plucker embedding used by 3D
            first_spatial_adapter_layer = DinoSpatialAdapter(embed_dim + plucker_emb_dim,
                                                            embed_dim,
                                                            spatial_reduction=spatial_reduction,
                                                            use_ffn=use_sa_ffn)
            spatial_adapters_layer = DinoSpatialAdapter(embed_dim,
                                                        embed_dim,
                                                        spatial_reduction=spatial_reduction,
                                                        use_ffn=use_sa_ffn)
            self.spatial_adapters = nn.ModuleList([first_spatial_adapter_layer] + [spatial_adapters_layer for _ in range(depth-1)])
    ...
    def forward_features(self, x: Tensor | List[Tensor],
                         ...
                         masks: Optional[Tensor] = None) -> List[Dict[str, Tensor]]:
        b, s = x.shape[:2]
        ...
        for i, blk in enumerate(self.blocks):
            ...
            x = blk(x, rope_sincos)
            if self.extended:
                spatial_tokens, extra_tokens = x[:, self.n_storage_tokens + 1:], x[:, :self.n_storage_tokens + 1]
                if i == 0:
                    input_spatial_tokens = self.add_plucker_embedding(spatial_tokens, pe, (b, s, H, W))
                else:
                    input_spatial_tokens = spatial_tokens
                input_sa_tokens = self.input_tokens_to_sa(input_spatial_tokens, extra_tokens)
                consistency_tokens = self.spatial_adapters[i](input_sa_tokens, (b, s, H, W))
                x = self.sa_to_output_tokens(spatial_tokens, extra_tokens, consistency_tokens)
        outputs = [x]
        outputs_normed = []
        for out in outputs:
        ...
        outputs = [
            out.reshape(b * s, H , W, -1).permute(0, 3, 1, 2).contiguous()
            for out in outputs
        ]
        out = rearrange(outputs[0], '(b s) c h w -> b s c h w', b=b, s=s)
        return out
