...
from dino3d.utils.utils import get_gt_warp
import wandb
import dino3d
import math

# This is slightly different than regular dino3d due to significantly worse corresps
...
class RobustLosses(nn.Module):
    ...
    def corr_volume_loss(self, mnn:torch.Tensor, corr_volume:torch.Tensor, scale):
        ...
        wandb.log(losses, step = dino3d.GLOBAL_STEP)
        return losses
    ...
    def regression_loss(self, x2, prob, flow, certainty, scale, eps=1e-8, mode = "delta"):
        ...
        if scale == 1:
            ...
            wandb.log({"train_pck_05": pck_05}, step = dino3d.GLOBAL_STEP)
