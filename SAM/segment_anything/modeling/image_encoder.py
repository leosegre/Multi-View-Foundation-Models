...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        if self.extended:
            print("Creating Transformer in extended SAM3D mode")
            # spatial adapters and plucker embedding used by CLIP3D
            if disable_plucker:
                plucker_emb_dim = 0
            else:
                self.plucker_embed = KorniaPatchEmbed(
    ...
    def forward(self, x: torch.Tensor, 
                camera_pe: torch.Tensor = None) -> torch.Tensor:
        b, s, _, _, _ = x.shape
        ...
        if self.extended:
            ...
            for i, block in enumerate(self.blocks):
                x = block(x)
                spatial_tokens = rearrange(x, '(b s) h w c -> (b s) (h w) c', b=b, s=s, h=h, w=w)
                if i == 0:
                    spatial_tokens = self.add_plucker_embedding(spatial_tokens, pe, (b, s, h, w))
                consistency_tokens = self.spatial_adapters[i](spatial_tokens, (b, s, h, w))
                consistency_tokens = rearrange(consistency_tokens, '(b s) (h w) c -> (b s) h w c', b=b, s=s, h=h, w=w)
                x = x + consistency_tokens
        else:
